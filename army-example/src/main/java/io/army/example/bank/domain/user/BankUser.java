...
public class BankUser<T extends BankUser<T>> extends BaseVersionDomain<T> {
    ...
    @Column(notNull = true, precision = 40, updatable = false, comment = "provide to partner user number")
    ...
    @Column(notNull = true, updatable = false, comment = "user certificate table id")
    private Long certificateId;

    @Column( comment = "user register complete time")
    private LocalDateTime completeTime;

    @Column(notNull = true, updatable = false, comment = "partner user id for person user,0 representing bank self.")
