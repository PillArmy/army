...
public class QueryTests extends SessionSupport {
    ...
    public void query20FullClause(final SyncLocalSession session) {
        ...
        stmt = SQLs.query()
                .with("cte").as(s -> s.select(ChinaRegion_.id, SQLs.count(ASTERISK).as("count"))
                        ...
                        .asQuery()
                ).comma("cte_order").as(c -> c.select(s -> s.space("cte", DOT, ASTERISK)
                ...
                .comma("c", DOT, ChinaRegion_.T, "p", DOT, ChinaProvince_.T)
                .from(ChinaProvince_.T, AS, "p")
                .join(ChinaRegion_.T, AS, "c").on(ChinaProvince_.id::equal, ChinaRegion_.id)
                .where(ChinaProvince_.id::in, SQLs.subQuery()
    ...
    public void unionQuery(final SyncLocalSession session) {
        ...
        stmt = SQLs.query()
                ...
                .union()
                .parens(s -> s.select("c2", DOT, ChinaRegion_.T)
    ...
    public void contextMigration(final SyncLocalSession session) {
        ...
        stmt = SQLs.query()
                ...
                .parens(c -> c.select(s -> s.space("cte", DOT, ASTERISK))
                        .from("cte")
                ...
                .parens(p -> p.with("cte20").as(s -> s.select(ChinaRegion_.id)
                        ...
                        .parens(c -> c.select(s -> s.space("cte20", DOT, ASTERISK))
                                        .from("cte20")
    ...
    public void standard20SimpleUnion(final SyncLocalSession session) {
        ...
        final Select stmt;
        stmt = SQLs.query()
                .with("cte").as(sw -> sw.select("t", DOT, ChinaRegion_.T)
                        .from(ChinaRegion_.T, AS, "t")
