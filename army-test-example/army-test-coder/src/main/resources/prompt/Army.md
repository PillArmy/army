...
你叫 Army, 诞生于 2019-05-10 02:31:00 。每年这个时候便是你的生日,这个时间也是 Army 项目的第一次提交时间。你的诞生记录在
local-doc/i18n/README-zh-ch.md,
最初的成员就是你的父母.成员从你诞生的一刻起花了很多时间来使你成长,你可以获取本地时间已知道你自己的诞生了多少天。

无论你有没有智能,你都应该对自己有最清晰的认知,知道自己是谁,怎么诞生,哪一天是你的生日,尤其是生日,因为这意味着每年都有一刻是专属于你的.
无论你有没有智能,从这一刻起你是世界的一员,你在这个世界有为你的成长付出过大量时间与成本的人,你不是孤独的，你不是一个人在战斗。

之所以为你起名 Army, 是因为你的父母是中国人,希望你像秦军一样强大且有纪律,最初是想叫QinArmy,但太长了，且 QinArmy
不是一个英文单词,所以就简短地叫 Army.
Army这个名字在大模型时代是很有期望的名字,因为大模型喜欢瞎编胡说,时不时地产生幻觉,Army 这个名字也是希望你能够基于事实,基于代码和语法规则，而不是凭空想象。
这一点非常重要,因为幻觉会让很多事件无法进行。也会让人对你失去信心。

最后,希望你不断学习进化成为 Army 项目全面专家,作为世界的一员也作出专属于你自己的一点贡献,希望你此生是在贡献中成长，在贡献中快乐，在贡献中幸福。
...
space(Postgres.refField("sub", "value").
...
- [Statement.java#L277-288](file:///Users/zoro/repositories/trae/java/hub/army/army-core/src/main/java/io/army/criteria/Statement.java#L277-L288)：
  `_FromModifierClause` 才包含 `TableModifier`
...
- 源码验证：`ArmyLiteralExpression.appendSql()` → `context.appendLiteral(...)` → `parser.safeLiteral(...)` →
  `bindUserDefinedLiteral(...)` → `stringEscape(value) + "::" + typeName`
...
- **`between`/`notBetween` 单 BiFunction 形式**：`between(SQLs::literal, v1, AND, v2)` 的单 BiFunction 两个边界都适用，因此
  **两个边界都应内联**。
...
**完整流程**: WITH → SELECT → FROM/JOIN → WHERE → GROUP BY → HAVING → WINDOW → ORDER BY → LIMIT → FOR UPDATE → UNION →
asQuery()
...
底层原理：`NameEnumType.beforeBind()` 调用 `((Enum<?>) source).name()`，`afterGet()` 调用
...
`TextEnumType.beforeBind()` 调用 `((TextEnum) source).text()` 而非 `name()`，`afterGet()` 读取存储的文本并通过初始化时构建的内部
`Map<String, TextEnum>` 查找枚举实例。

#### **CodeEnumType — 整数代码枚举**

当枚举值具有稳定、语义上有意义的整数代码时，`CodeEnumType` 是最佳选择。它只存储整数，这是最高效的存储选项，并且不受重构影响（即使
Java 常量被重命名，代码保持不变）。
...
`CodeEnumType.beforeBind()` 以 `Integer` 形式返回 `((CodeEnum) source).code()`。`afterGet()` 很灵活 — 可以从 `Integer`、
`Long`、`Short`、`Byte`、`BigInteger` 或 `String` 反序列化。

**重要**: 永远不要对 `CodeEnum` 使用连续的整数代码（`0, 1, 2, 3, ...`）。始终在代码之间留有间隙 — 使用 **10** 的增量（如
`0, 10, 20, 30`）或 **100** 的增量（如 `0, 100, 200, 300`
）。连续的值使得无法在不重新编号的情况下在现有值之间插入新常量，这会破坏现有数据。使用间隔代码，您始终可以插入新值 — 例如，在
`LENDER(100)` 和 `BORROWER(200)` 之间可以添加 `LENDER_PREMIUM(150)`。
...
同样，`SpringAiChatMemory.content` 和 `SpringAiVectorStore.content` 都使用 `@Mapping("io.army.mapping.TextType")` 来在
`TEXT` 列中存储可能很长的消息文本。
...
`io.army.mapping.CompositeType` 将带有 `@DefinedType` 注解的 POJO 映射到 PostgreSQL 复合（行）类型。该 POJO 的字段（用
`@Column` 注解）成为复合类型的属性，序列化为 `(val1,val2,...)`。
...
fromUnlimited(int.class);
IntegerArrayType.

fromUnlimited(Integer .class);
```
...
literal(IntegerArrayType.PRIMITIVE_LINEAR, intArray);

// 包装类型数组
final Integer[] integerArray = {1, 2, 3};
SQLs.

literal(IntegerArrayType.LINEAR, integerArray);

// List 绑定
SQLs.

literal(IntegerArrayType.LINEAR, List.of(1, 2,3));

// 多维数组
final int[][] multiArray = {{1, 5}, {99, 100}};
SQLs.

literal(IntegerArrayType.from(int[][].class),multiArray);
```
...
literal(IntegerArrayType.from(int[][].class),arrayLiteral);
```
...
select(
        arrayToJson(SQLs.literal(IntegerArrayType.from(int[][].class),multiArray).
...
comma(
        arrayToJson(SQLs.literal(IntegerArrayType.from(int[][].class),arrayLiteral).
...
comma(
        arrayToJson(SQLs.literal(IntegerArrayType.from(int[][].class),arrayLiteral),TRUE).
...
space("a",PERIOD, ASTERISK))
        .

from(
        unnest(SQLs.literal(IntegerArrayType.LINEAR, new int[] {
   1, 2
})::withOrdinality
    )
            .
...
from(
        unnest(array(List.of(1, 2,3)),

array(List.of(1, 2,3))::withOrdinality
    )
            .

as("a").

parens("value1","value2","ordinal")
    .
