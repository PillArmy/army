---
name: "postgres-batch-delete"
description: "PostgreSQL 批量单表删除方法链完整文档，包含所有可用方法、参数、使用场景和示例。Invoke when user needs to use or understand Postgres.batchSingleDelete() method chain."
---

# Postgres.batchSingleDelete() 方法链完整文档

## 概述

`Postgres.batchSingleDelete()` 是 Army Criteria 框架中用于构建 PostgreSQL 批量单表 DELETE 语句的流式 API。该方法链支持复杂的删除操作，包括：
...
Postgres.batchSingleDelete()
  ...
  └─ 结束语句
        ...
        └─ [asReturningDelete() → _BatchReturningDeleteParamSpec]
           └─ namedParamList(List)
```
...
```java
PgSingleDeleteSpec<_BatchDeleteParamSpec, _BatchReturningDeleteParamSpec> batchSingleDelete()
```

**说明**: 创建批量单表 DELETE 语句构建器，这是整个方法链的起点。
...
- `star`: 使用 `SQLs.ASTERISK` 显式指定删除本表和所有继承表
...
```java
_BatchDeleteParamSpec asDelete()
```

返回 `_BatchDeleteParamSpec`，然后可以调用 `namedParamList(List)`。
...
```java
_BatchReturningDeleteParamSpec asReturningDelete()
```

返回 `_BatchReturningDeleteParamSpec`，然后可以调用 `namedParamList(List)`。
...
```java
Postgres.batchSingleDelete()
  .deleteFrom(UserTable.T, SQLs.AS, "u")
  .where(UserTable.id.in(SQLs::rowParam, extractIds(userList)))
  .asDelete()
  .namedParamList(userList);
```

---

### 示例 2: 带 RETURNING 的批量删除
...
Postgres.batchSingleDelete()
  .deleteFrom(UserTable.T, SQLs.AS, "u")
  .using(RoleTable.T, SQLs.AS, "r")
  .where(UserTable.roleId.eq(RoleTable.id))
  .and(RoleTable.name.eq(SQLs::rowParam, extractRoleName(userList)))
  .asDelete()
  .namedParamList(userList);
```
...
Postgres.batchSingleDelete()
  ...
  .as(Postgres.query()
      .select(UserTable.id)
      .from(UserTable.T, SQLs.AS, "u")
      .where(UserTable.status.eq("inactive"))
      .asSubQuery())
  .deleteFrom(UserTable.T, SQLs.AS, "u")
  .using("to_delete", SQLs.AS, "td")
  .where(UserTable.id.eq(Cte.ref("to_delete", UserTable.id)))
  .asDelete()
...
Postgres.batchSingleDelete()
  .deleteFrom(UserTable.T, SQLs.AS, "u")
  .whereIf(UserTable.status::eq, statusParam)
  .ifAnd(UserTable.createdAt::lt, dateParam)
  .asDelete()
  .namedParamList(params);
```
...
2. **使用批量参数**: 通过 `namedParamList()` 传递批量参数列表
3. **条件性子句**: 使用 `ifXxx` 方法进行条件性添加子句
4. **RETURNING 子句**: 需要返回删除的数据时使用
5. **安全性**: DML 语句必须有 WHERE 子句，框架会进行验证
...
- `_BatchDeleteParamSpec`: 批量删除参数规范
- `_BatchReturningDeleteParamSpec`: 带 RETURNING 的批量删除参数规范
