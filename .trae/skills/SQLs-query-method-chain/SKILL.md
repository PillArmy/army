...
> AND、GROUP BY、HAVING、WINDOW、ORDER BY、LIMIT、FOR UPDATE、UNION、
...
> **注意**: INTERSECT / EXCEPT / MINUS 方法**不**在标准 `SQLs.query()` 链中。它们定义在 `RowSet._StaticIntersectClause` / `_StaticExceptClause` / `_StaticMinusClause` 中，仅由方言特定的 Query 接口（如 `MySQLQuery`、`PostgreQuery`、`OracleQuery`）提供。
...
| `_StaticSelectSpaceClause`   | `space(...)`                                                  | `_StandardSelectCommaClause`    | —                        |
...
| 条件 | 应使用的形式 | 示例 |
|------|------------|------|
| selections 确定 + 不引用 FROM | **Static** | `.select(Stock_.id, Stock_.name)` |
| selections 确定 + 需要引用 FROM 导出表/列 | **Defer** | `.select(s -> s.space(refField("bu", ID)))` |
| selections 不确定 | **Dynamic** | `.selects(consumer -> { consumer.selection(...); })` |

> **注意**：Defer 的 `_DeferSelectSpaceClause` 和 Dynamic 的 `SelectionConsumer` 都继承 `_DeferContextSpec`，
> 通过 `ContextStack.peek()` 提供延迟上下文解析。Static 形式的纯 `Selection` 不需要此标记。
> `Function<String,Selection>` 在 `_StaticSelectClause` 中，Function 被**立即调用**而非延迟，不提供 `_DeferContextSpec`，
> 故不适用于 `refField`。
...
默认 `LiteralMode.DEFAULT`，普通 Java 值自动参数化为 JDBC `?` 占位符：

```java
// ✅ 无需显式 SQLs::param 或 SQLs.parameter()
Stock_.offerPrice.greater(1000)               // → WHERE offer_price > ?

// TypedField 有两个 equal 方法
// ① TypedExpression 上的显式参数绑定（不受 LiteralMode 影响）
.where(Stock_.status.equal(SQLs::param, value))
// ② Expression 上的隐式参数绑定（受 LiteralMode 影响）
.where(Stock_.status.equal(value))

// DerivedField（refField）只有一个 equal 方法（继承自 Expression）
// ① 显式参数绑定
.where(refField("us", "one").equal(SQLs.parameter(1)))
// ② 隐式参数绑定（受 LiteralMode 影响）
.where(refField("us", "one").equal(1))
```
...
> `Expression.as(String)` 返回 `NamedExpression`，与 `::as` 方法引用等效。

**DEFAULT Mode 消除冗余的 `SQLs.parameter()` / `SQLs::param` / `SQLs::literal`**：

```java
// ❌ 冗余
.where(Stock_.id.equal(SQLs::literal, 1))
.and(Stock_.name.equal(SQLs::param, "test"))
.limit(SQLs::literal, 0, 10)

// ✅ 简洁
.where(Stock_.id.equal(1))
.and(Stock_.name.equal("test"))
.limit(0, 10)
```

> 仅当需要**强制字面量**（防参数化）或**强制参数化**（防字面量注入）时才显式使用 `SQLs::literal` / `SQLs::param`。
...
Select stmt = SQLs.query()
        .select(s -> s.space(SQLs.refField("us", "one"))
                ...
                .select(SQLs.literalValue(1).as("one"))   // .as() 优于 ::as
                .from(PillUser_.T, AS, "u")
                .asQuery()
        ).as("us")
        .where(SQLs.refField("us", "one").equal(1))        // DEFAULT mode 自动参数化
        .asQuery();
```
